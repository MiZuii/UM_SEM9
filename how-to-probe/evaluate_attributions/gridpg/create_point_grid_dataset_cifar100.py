#!/usr/bin/env python3
"""
Create GridPG images from confident CIFAR100 images.
Creates 2x2 and 3x3 grids of different class images for GridPG evaluation.

Usage:
    python create_point_grid_dataset_cifar100.py --input_dir confident_images_cifar100_byol
"""
import os
import argparse
import random
...
def main():
    parser = argparse.ArgumentParser(description='Create GridPG images for CIFAR100')
    ...
    # Load images grouped by class (100 classes for CIFAR100)
    num_classes = 100
    ...
    classes_with_images = [i for i in range(num_classes) if len(class_images[i]) > 0]
    print(f"Found images for {len(classes_with_images)}/{num_classes} classes")

    # Create 2x2 grids (4 images, 4 classes)
    print(f"\nCreating {args.num_samples} 2x2 grid images...")
    count_2x2 = 0
    for sample_idx in range(args.num_samples):
        # Select 4 random classes from those with images
        if len(classes_with_images) < 4:
            print("Warning: Not enough classes with images for 2x2 grids")
            break
        selected_classes = random.sample(classes_with_images, 4)
        images = []
        labels = []
    ...
    print(f"  Saved {count_2x2} 2x2 grids to {args.output_dir_2x2}")

    # Create 3x3 grids (9 images, 9 classes)
    print(f"\nCreating {args.num_samples} 3x3 grid images...")
    count_3x3 = 0
    for sample_idx in range(args.num_samples):
        # Select 9 random classes from those with images
        if len(classes_with_images) < 9:
            selected_classes = random.choices(classes_with_images, k=9)
        else:
            selected_classes = random.sample(classes_with_images, 9)
        images = []
        labels = []
    ...
    print(f"  Saved {count_3x3} 3x3 grids to {args.output_dir_3x3}")
    print("\nDone!")
