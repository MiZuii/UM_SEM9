...
def create_grid_sample_2x2(ord_images, no_of_classes, base_dir):
    ...
    available_classes = [c for c in ord_images if len(ord_images[c]) > 0]
    
    if len(available_classes) == 0:
        raise ValueError("No confident images found!")

    while len(sel_classes) < 4:
        curr_class = random.choice(available_classes)
        
        # Only enforce uniqueness if we have at least 4 available classes
        if len(available_classes) >= 4:
            if curr_class in sel_classes:
                continue

        sel_classes.append(curr_class)
        curr_idx = random.randint(0, len(ord_images[curr_class])-1)
        
        img_path = os.path.join(base_dir, ord_images[curr_class][curr_idx])
    ...
    sel_classes_str = [str(x) for x in sel_classes]
    sel_classes_str = '_'.join(sel_classes_str)
...
def create_grid_sample_3x3(ord_images, no_of_classes, base_dir):
    ...
    # 1. Get a list of ALL available classes that actually have images
    available_classes = [c for c in ord_images if len(ord_images[c]) > 0]
    
    if len(available_classes) == 0:
        raise ValueError("No confident images found! Cannot create grid.")

    while len(sel_classes) < 9:
        # 2. Pick a class from what is actually available
        curr_class = random.choice(available_classes)
        
        # 3. RELAXED CONDITION: 
        # Only enforce uniqueness if we have enough classes to support it.
        ...
        if img is None:
            # If image load fails, remove this attempt and try again
            sel_classes.pop()
            continue
...
base_dir = 'cifar10_confident_images/'
raw_images = os.listdir(base_dir)
#raw_images = [x for x in raw_images if 'sup' not in x]
number_of_classes = -1
class_to_image_dict = {}
for image in tqdm(raw_images, desc="Grouping images"):
...
os.makedirs(output_dir_2x2, exist_ok=True)
os.makedirs(output_dir_3x3, exist_ok=True)
