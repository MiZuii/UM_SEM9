#!/usr/bin/env python3
"""
Generate Figure 7-style visualization: MLP vs Linear Probe comparison for CIFAR100.

Layout:
- Rows: Input Image | Linear Probe | 3-layer MLP
- Columns: Different CIFAR100 classes
- Attribution maps show dotted rectangles highlighting the attended region

Usage:
    python visualize_probe_comparison_cifar100.py --num_samples 5
    python visualize_probe_comparison_cifar100.py --model byol
    python visualize_probe_comparison_cifar100.py --model dino
"""
import os
import argparse
import numpy as np
...
# Model checkpoint paths (using linear probe checkpoints)
MODEL_CONFIGS = {
    'byol': {
...
def build_linear_probe(checkpoint_path, device):
    ...
    model = LinearProbe(backbone, num_classes=100, in_channels=2048)
...
def create_probe_comparison_figure(linear_model, mlp_model, sample_images, sample_labels, 
                                    device, norm_fn, output_path, model_name='BYOL'):
    """
    ...
    # Adjust layout
    plt.subplots_adjust(left=0.12, right=0.95, top=0.88, bottom=0.05, wspace=0.1, hspace=0.15)
    ...
    for col, (img, label) in enumerate(zip(sample_images, sample_labels)):
        ...
        axes[0, col].set_title(class_name, fontsize=11, fontweight='bold', pad=8)
        axes[0, col].axis('off')
    ...
    # Add figure title
    plt.suptitle(f'{model_name}: MLP vs Linear Probe (CIFAR100)\n'
...
def main():
    parser = argparse.ArgumentParser(description='Generate probe comparison figure for CIFAR100')
    parser.add_argument('--model', type=str, default='byol', choices=['byol', 'dino', 'mocov2'],
    ...
    checkpoint_path = args.checkpoint if args.checkpoint else model_config['checkpoint']
    model_name = model_config['name']
    
    # Load models
    print(f"Loading Linear Probe model ({model_name})...")
    linear_model = build_linear_probe(checkpoint_path, device)
    
    print(f"Loading MLP Probe model ({model_name})...")
    # Note: Using same model for demonstration; replace with actual MLP probe
    mlp_model = build_mlp_probe(checkpoint_path, device)
    ...
    # Select diverse samples - pick from well-known classes for better visualization
    # Prefer recognizable classes like animals, vehicles
    ...
    print(f"Selected {len(sample_images)} samples: {[CIFAR100_CLASSES[l] for l in sample_labels]}")
    
    # Generate comparison figure
    output_path = os.path.join(args.output_dir, f'probe_comparison_cifar100_{args.model}.png')
    create_probe_comparison_figure(linear_model, mlp_model, sample_images, sample_labels,
                                   device, norm_fn, output_path, model_name=model_name)
