#!/bin/bash
# DINO Pretraining on CIFAR10 with 2x RTX A4000 GPUs
...
DATA_DIR="${PROJECT_ROOT}/data/cifar10_imagefolder"
OUTPUT_DIR="${PROJECT_ROOT}/outputs/dino_cifar10"

# Training hyperparameters (downsized for CIFAR10 and 2x A4000)
EPOCHS=50
BATCH_SIZE_PER_GPU=128
NUM_WORKERS=4
LR=0.003

# CIFAR10-specific crop sizes (32x32 images)
GLOBAL_CROP_SIZE=32
LOCAL_CROP_SIZE=14
...
if [ ! -d "${DATA_DIR}" ]; then
    ...
    echo "Please run: python ${SCRIPT_DIR}/download_cifar10.py --output ${DATA_DIR}"
    exit 1
fi
